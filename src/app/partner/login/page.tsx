...
import { Eye, EyeOff, LogIn, Loader2, Users, ArrowLeft } from 'lucide-react';
...
export default function PartnerLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const response = await partnerService.login({ phone: `+91${phone}`, password });
      if (response.success && response.data) {
        const { token, partner } = response.data;
        localStorage.setItem(TOKEN_KEYS.partner, token);
        localStorage.setItem(USER_KEYS.partner, JSON.stringify(partner));
        ...
        <div className="p-8 rounded-3xl bg-white/5 backdrop-blur-xl border border-white/10 shadow-2xl">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              <label htmlFor="partner-phone" className="text-sm font-medium text-neutral-300 ml-1">
                Phone Number
              </label>
