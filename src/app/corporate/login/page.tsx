...
import { Eye, EyeOff, LogIn, Loader2, Building2, ArrowLeft } from 'lucide-react';
...
export default function CorporateLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const response = await corporateService.login({ email, password });
      if (response.success && response.data) {
        const { token, corporate } = response.data;
        localStorage.setItem(TOKEN_KEYS.corporate, token);
        localStorage.setItem(USER_KEYS.corporate, JSON.stringify(corporate));
    ...
    <div className="min-h-screen bg-[#0D0D0D] flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md animate-fade-in">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-bold text-white mb-3">Corporate Login</h1>
          <p className="text-neutral-400">Sign in to manage corporate bookings</p>
        </div>
